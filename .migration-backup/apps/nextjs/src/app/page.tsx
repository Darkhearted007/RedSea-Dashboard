...
export default function Home() {
  ...
  useEffect(() => {
    ...
    ws.onopen = () => {
      if (key) {
        ws.send(JSON.stringify({
          APIKey: key,
          BoundingBoxes: [[[0, 0], [1, 1]]], // tiny box — minimal data
    ...
    ws.onmessage = (e) => {
      try {
        ...
        if (msg.MessageType === "PositionReport") {
          ...
          ws.close() // one message is enough to confirm
        }
      } catch {}
    }
    ...
    const wsTimeout = setTimeout(() => {
      if (status.ais === "checking" || status.ais === "pending") {
        setStatus(s => ({ ...s, ais: "pending" }))
      }
      ws.close()
    }, 8000)

    // 3. Check AI engine (Claude API via our endpoint)
    fetch("/api/health/ai")
